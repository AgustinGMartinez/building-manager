...
function validateAssignment(assignment) {
  ...
  const {
    error
  } = Joi.validate(assignment, schema);
...
const getAssignments = async ({
  ...
  const buildingsQuery = `
    ...
    from buildings
  `;
  const [assignments, doorbellsAssignments, buildings] = await Promise.all([query(assignmentsQuery, [id, campaign_id, user_id]), query(doorbellsAssignmentsQuery, [id]), query(buildingsQuery)]);
  await Promise.all(assignments.rows.map(async assignment => {
    ...
    doorbellsAssignments.rows.forEach(doorbellAssignment => {
      if (doorbellAssignment.assignment_id === assignmentId) {
        aDoorbells.push({ ...doorbellAssignment,
    ...
    if (user_id) {
      ...
      const {
        rows
      } = await query(usersWithSameBuildingsAssignedQuery, [user_id, assignment.buildings.map(b => b.id)].flat());
      const users = [];

      if (rows.length) {
        rows.forEach(row => {
          const fullname = `${row.name} ${row.lastname}`;
          if (!users.some(u => u.id === row.user_id)) users.push({
            fullname,
            id: row.user_id
          });
        });
...
router.get('/:id', authenticated.user, async (req, res) => {
  try {
    const assignment = await getAssignments({
      id: req.params.id
    });
    res.send(assignment);
  } catch (err) {
...
router.post('/', authenticated.admin, async (req, res, next) => {
  try {
    ...
    const {
      rows
    } = await query(assignmentQuery, [note, expiry_date, campaign_id]);
    ...
    const partialDoorbellsAssignmentsQuery = `
      ...
      VALUES ?
    `;
    const doorbellsAssignmentsQuery = createQueryValues(partialDoorbellsAssignmentsQuery, doorbellsAssignmentsData, 3);
...
router.put('/:id', authenticated.user, async (req, res, next) => {
  try {
    ...
    const assignmentId = req.params.id; // validate assignmet id belonging to user
    ...
    if (!isAdmin && assignment.user_id !== userId) throw new CustomError(403, 'Sin trampas please'); // if assignment is complete, ignore petition

    if (assignment.completed) throw new CustomError(400, "Can't modify completed assignment"); // toogle boorbell
    ...
    `;
    await query(toggleDoorbellQuery, [assignmentId, specialId]); // update building last done
    ...
    const updateBuildingLastDoneQuery = `
      ...
      WHERE id = $1
    `;
    await query(updateBuildingLastDoneQuery, [buildingId]); // TODO: revert building last done if user toggles off building
    ...
    const inHistory = result.rows.length === 1; // update history
